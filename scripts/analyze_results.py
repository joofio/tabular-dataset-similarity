...
def plot_ranking_comparison(df: pd.DataFrame, output_dir: str = "results"):
    ...
    ranking_cols = ["kendalltau", "weightedtau", "spearmanr", "ndcg_score", "rbo"]
...
def plot_trts_comparison(df: pd.DataFrame, output_dir: str = "results"):
    ...
    if "trtr_accuracy" in df.columns and "trts_accuracy" in df.columns:
        df_class = df[df["task_type"] == "classification"].dropna(subset=["trtr_accuracy", "trts_accuracy"])
        ...
        if len(df_class) > 0:
            ...
            for perturbation in df_class["perturbation"].unique():
                ...
                axes[0].scatter(subset["trtr_accuracy"], subset["trts_accuracy"],
                              label=perturbation, alpha=0.6)
            ...
            df_class["accuracy_diff"] = df_class["trtr_accuracy"] - df_class["trts_accuracy"]
    ...
    if "trtr_rmse" in df.columns and "trts_rmse" in df.columns:
        df_reg = df[df["task_type"] == "regression"].dropna(subset=["trtr_rmse", "trts_rmse"])

        if len(df_reg) > 0:
            fig, ax = plt.subplots(figsize=(10, 5))
            df_reg["rmse_ratio"] = df_reg["trts_rmse"] / df_reg["trtr_rmse"].replace(0, np.nan)
            sns.boxplot(data=df_reg, x="perturbation", y="rmse_ratio", ax=ax)
            ax.axhline(1, color="red", linestyle="--", alpha=0.5, label="Equal performance")
...
def plot_aggregated_with_ci(df: pd.DataFrame, output_dir: str = "results"):
    ...
    df_agg = df_filtered.groupby(["perturbation", "metric"]).agg({
        "median": "mean",
        "ci_lower": "mean",
        "ci_upper": "mean",
        "n": "sum"
    ...
    for i, metric in enumerate(metrics):
        ...
        ci_lowers = [metric_dict.get(p, {}).get("ci_lower", np.nan) for p in perturbations]
        ci_uppers = [metric_dict.get(p, {}).get("ci_upper", np.nan) for p in perturbations]

        yerr_lower = [m - l if not np.isnan(m) and not np.isnan(l) else 0
                      for m, l in zip(medians, ci_lowers)]
        yerr_upper = [u - m if not np.isnan(m) and not np.isnan(u) else 0
                      for m, u in zip(medians, ci_uppers)]

        ax.bar(x + i * width, medians, width,
               label=metric, yerr=[yerr_lower, yerr_upper], capsize=2)
...
def plot_perturbation_summary(df_baseline: pd.DataFrame, df_ranking: pd.DataFrame,
    ...
    if "propensity_auroc" in df_baseline.columns:
        prop_data = df_baseline.groupby("perturbation")["propensity_auroc"].agg(["mean", "std"])
        prop_data = prop_data.sort_values("mean")

        axes[0].barh(prop_data.index, prop_data["mean"], xerr=prop_data["std"], capsize=3)
        axes[0].axvline(0.5, color="green", linestyle="--", alpha=0.7, label="Indistinguishable")
        axes[0].set_xlabel("Propensity AUROC")
        axes[0].set_title("Dataset Discriminability\n(0.5 = identical, 1.0 = perfectly distinguishable)")
        axes[0].legend()
    ...
    if not df_ranking.empty:
        ...
        if available:
            rank_mean = df_ranking.groupby("perturbation")[available].mean().mean(axis=1)
            ...
            axes[1].set_title("Feature Importance Agreement\n(1.0 = identical rankings)")
...
def compute_ranking_utility_correlation(df_target: pd.DataFrame, df_ranking: pd.DataFrame,
                                         output_dir: str = "results"):
    """
    ...
    predict actual utility loss (TRTR - TRTS performance)?
    """
    output_path = Path(output_dir) / "figures"
    output_path.mkdir(exist_ok=True)
    ...
    available_keys = [k for k in merge_keys if k in df_ranking.columns and k in df_target.columns]
    ...
    # For classification: accuracy drop = TRTR - TRTS (positive = worse on perturbed)
    if "trtr_accuracy" in df_merged.columns and "trts_accuracy" in df_merged.columns:
        df_merged["accuracy_drop"] = df_merged["trtr_accuracy"] - df_merged["trts_accuracy"]
    if "trtr_balanced_accuracy" in df_merged.columns and "trts_balanced_accuracy" in df_merged.columns:
        df_merged["balanced_accuracy_drop"] = df_merged["trtr_balanced_accuracy"] - df_merged["trts_balanced_accuracy"]

    # For regression: RMSE increase ratio = TRTS/TRTR (>1 = worse on perturbed)
    if "trtr_rmse" in df_merged.columns and "trts_rmse" in df_merged.columns:
        df_merged["rmse_ratio"] = df_merged["trts_rmse"] / df_merged["trtr_rmse"].replace(0, np.nan)

    # Ranking metrics to correlate
    ranking_cols = ["kendalltau", "weightedtau", "spearmanr", "rbo", "ndcg_score"]
    available_ranking = [c for c in ranking_cols if c in df_merged.columns]

    # Utility metrics to correlate against
    utility_cols = ["accuracy_drop", "balanced_accuracy_drop", "rmse_ratio"]
    available_utility = [c for c in utility_cols if c in df_merged.columns]

    if not available_ranking or not available_utility:
        print(f"Insufficient metrics for correlation: ranking={available_ranking}, utility={available_utility}")
        return None
    ...
    for rank_metric in available_ranking:
        for util_metric in available_utility:
            ...
            spearman_r, spearman_p = st.spearmanr(subset[rank_metric], subset[util_metric])
    ...
    print("(Negative correlation = higher ranking similarity predicts lower utility loss)")
...
def plot_ranking_vs_utility(df_merged: pd.DataFrame, output_dir: str = "results"):
    ...
    # Primary comparison: Kendall's tau vs accuracy drop
    ranking_cols = ["kendalltau", "rbo", "spearmanr"]
    available_ranking = [c for c in ranking_cols if c in df_merged.columns]

    # Classification utility
    if "accuracy_drop" in df_merged.columns and available_ranking:
        df_class = df_merged.dropna(subset=["accuracy_drop"] + available_ranking[:1])

        if len(df_class) > 10:
            fig, axes = plt.subplots(1, min(len(available_ranking), 3), figsize=(5 * min(len(available_ranking), 3), 5))
            ...
            for ax, rank_col in zip(axes, available_ranking[:3]):
                for perturb in df_class["perturbation"].unique():
                    ...
                    ax.scatter(subset[rank_col], subset["accuracy_drop"],
                              label=perturb, alpha=0.6, s=30)

                # Add trend line
                valid = df_class[[rank_col, "accuracy_drop"]].dropna()
                if len(valid) > 5:
                    z = np.polyfit(valid[rank_col], valid["accuracy_drop"], 1)
                    p = np.poly1d(z)
                    x_line = np.linspace(valid[rank_col].min(), valid[rank_col].max(), 100)
                    ax.plot(x_line, p(x_line), "k--", alpha=0.5, label="Trend")

                    # Add correlation coefficient
                    r, pval = st.pearsonr(valid[rank_col], valid["accuracy_drop"])
                ...
                ax.set_ylabel("Accuracy Drop (TRTR - TRTS)")
                ax.set_title(f"Ranking Similarity vs Utility Loss")
    ...
    # Regression utility
    if "rmse_ratio" in df_merged.columns and available_ranking:
        df_reg = df_merged.dropna(subset=["rmse_ratio"] + available_ranking[:1])
        # Filter extreme ratios
        df_reg = df_reg[(df_reg["rmse_ratio"] > 0.1) & (df_reg["rmse_ratio"] < 10)]
        ...
        if len(df_reg) > 10:
            ...
            for ax, rank_col in zip(axes, available_ranking[:3]):
                for perturb in df_reg["perturbation"].unique():
                    ...
                    ax.scatter(subset[rank_col], subset["rmse_ratio"],
                              label=perturb, alpha=0.6, s=30)

                # Add trend line
                valid = df_reg[[rank_col, "rmse_ratio"]].dropna()
                if len(valid) > 5:
                    z = np.polyfit(valid[rank_col], valid["rmse_ratio"], 1)
                    p = np.poly1d(z)
                    x_line = np.linspace(valid[rank_col].min(), valid[rank_col].max(), 100)
                    ax.plot(x_line, p(x_line), "k--", alpha=0.5, label="Trend")

                    r, pval = st.pearsonr(valid[rank_col], valid["rmse_ratio"])
                    ax.text(0.05, 0.95, f"r={r:.3f}, p={pval:.3f}",
                           transform=ax.transAxes, fontsize=10, va="top")

                ax.axhline(1, color="green", linestyle="--", alpha=0.3, label="No degradation")
                ax.set_xlabel(f"{rank_col} (higher = more similar)")
                ax.set_ylabel("RMSE Ratio (TRTS/TRTR)")
                ax.set_title(f"Ranking Similarity vs Utility Loss")
...
def compare_metrics_as_utility_predictors(df_merged: pd.DataFrame, df_baseline: pd.DataFrame,
                                           output_dir: str = "results"):
    """
    ...
    df_all = df_merged.merge(df_baseline, on=merge_keys, how="left", suffixes=("", "_baseline"))

    # All candidate predictor metrics
    ranking_predictors = ["kendalltau", "rbo", "spearmanr", "weightedtau"]
    baseline_predictors = ["propensity_auroc", "marginal_js_distance", "mmd_rbf"]

    available_predictors = [c for c in ranking_predictors + baseline_predictors if c in df_all.columns]

    utility_metrics = ["accuracy_drop", "balanced_accuracy_drop", "rmse_ratio"]
    ...
    df_agg = df_scores.groupby(["predictor", "type"]).agg({
        "abs_spearman": "mean",
        "n": "sum"
    }).reset_index().sort_values("abs_spearman", ascending=False)
